...
public class InventarioAula {
	...
	/**
	 * Elimina un puesto del inventario
	 * @param códigoPuesto el código del puesto a eliminar
	 * @throws InventarioException si el código no existe
	 */
	public void eliminarPuesto(String códigoPuesto) throws InventarioException {
	    códigoPuesto = normalizarTexto(códigoPuesto, "Código de puesto requerido");
	    códigoPuesto = códigoPuesto.toUpperCase(Locale.of("es", "ES"));
	    
	    if (!listaPuestos.containsKey(códigoPuesto)) {
	        throw new InventarioException("El código " + códigoPuesto + " no existe en el inventario");
	    }
	    
	    if (!VistaGeneral.pedirConfirmación("Confirmar eliminación del puesto " + códigoPuesto, 
	                                       "Esta acción no se puede deshacer")) {
	        throw new InventarioException("Eliminación cancelada por el usuario");
	    }
	...
	public List<String> generarListadoCSV() {
		...
		listaExportable.add(0, prefijoAula);
	...
	/**
	 * Vacía completamente el inventario, eliminando todos los puestos registrados
	 * en la colección.
	 * 
	 * Este método no realiza ninguna validación, simplemente elimina todas las
	 * entradas del mapa de puestos.
	 */
	public void vaciar() {
		listaPuestos.clear();
	}
